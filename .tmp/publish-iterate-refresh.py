...
def refresh_aws_credentials():
    try:
        # Refresh AWS credentials by creating a new session
        global session
        session = boto3.Session()
        credentials = session.get_credentials()
        global awsauth
...
def metrics():
    ...
    refresh_aws_credentials()  # Refresh AWS credentials before fetching data
    fetch_data()  # Fetch data before exposing metrics
    return generate_latest(REGISTRY)

if __name__ == '__main__':
    scheduler = BackgroundScheduler()
    scheduler.add_job(refresh_aws_credentials, 'interval', seconds=600)  # Schedule credential refresh every 10 minutes
    scheduler.add_job(fetch_data, 'interval', seconds=60)  # Schedule data fetch every 60 seconds
    scheduler.start()
